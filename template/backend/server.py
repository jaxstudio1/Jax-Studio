...
SENDER_EMAIL = os.environ.get('SENDER_EMAIL', 'hello@template.test')
SENDER_NAME = os.environ.get('SENDER_NAME', 'Studio Name')
OWNER_EMAIL = os.environ.get('OWNER_EMAIL', 'hello@template.test')
...
# Uploads
UPLOAD_DIR = Path(os.environ.get('UPLOAD_DIR', '/app/template/backend/uploads'))
...
# App + router
app = FastAPI(title="WebGL Cube Template API")
...
class Settings(BaseModel):
    ...
    cube_letter_spacing: Optional[float] = None
    cube_line_spacing: Optional[float] = None
...
def _owner_email_html(name, email, message, submission_id):
    ...
    <html><body style="margin:0;padding:0;background:#0a0a0a;font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;color:#eaeaea;">
      <div style="max-width:560px;margin:0 auto;padding:32px 28px;">
        <div style="font-size:11px;letter-spacing:0.4em;text-transform:uppercase;color:#888;margin-bottom:24px;">{html_lib.escape(SENDER_NAME).upper()} · NEW INQUIRY</div>
...
def _autoreply_email_html(name, email, message):
    ...
    <html><body style="margin:0;padding:0;background:#0a0a0a;font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;color:#eaeaea;">
      <div style="max-width:560px;margin:0 auto;padding:32px 28px;">
        <div style="font-size:11px;letter-spacing:0.4em;text-transform:uppercase;color:#888;margin-bottom:24px;">{html_lib.escape(SENDER_NAME).upper()}</div>
        ...
        <p style="margin-top:30px;font-size:12px;color:#666;">— {html_lib.escape(SENDER_NAME)} · {html_lib.escape(SENDER_EMAIL)}</p>
      </div>
    </body></html>
    """
...
def send_contact_emails(name, email, message, submission_id):
    ...
    try:
        owner_mail = Mail(
            ...
            subject=f"New inquiry from {name} — {SENDER_NAME}",
...
@api_router.get("/")
async def root():
    return {"message": "WebGL Cube Template API up"}
