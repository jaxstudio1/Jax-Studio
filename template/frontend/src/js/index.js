...
/**
 * Click cube → reveal the site behind the curtain.
 * The landing curtain is a fixed full-viewport layer; tagging it with
 * `.is-revealed` triggers a scale-up + fade + blur animation. Once it
 * finishes, we un-lock body scroll so the visitor can explore the page
 * below. A small "back to intro" button in the footer brings the cube
 * back if anyone wants to see it again.
 */
const curtain = document.querySelector('.landing-curtain')
const hintEl = document.querySelector('.hint')

const revealSite = () => {
  if (!curtain || curtain.classList.contains('is-revealed')) return
  curtain.classList.add('is-revealed')
  if (hintEl) hintEl.style.opacity = '0'
  // unlock body scrolling once the animation has visibly started
  setTimeout(() => {
    document.body.classList.remove('is-locked')
    // Smoothly snap to the very top of the site content so the hero is
    // what the visitor sees first after the reveal completes.
    window.scrollTo({ top: 0, behavior: 'instant' in window ? 'instant' : 'auto' })
  }, 80)
}

const restoreCurtain = () => {
  if (!curtain) return
...
// Lock body scroll on initial paint — site content shouldn't scroll while
// the curtain is up.
document.body.classList.add('is-locked')

stageEl.addEventListener('click', () => revealSite())

// Quick-tap on mobile (touchend < 350ms with little movement)
let touchStart = null
stageEl.addEventListener('touchstart', (e) => {
...
stageEl.addEventListener('touchend', (e) => {
  ...
  if (last && dt < 350) {
    ...
    if (dx < 12 && dy < 12) revealSite()
  }
  touchStart = null
})

// "Back to intro" button in the site footer
const backBtn = document.querySelector('[data-testid="site-back-to-intro"]')
if (backBtn) backBtn.addEventListener('click', restoreCurtain)
...
// "Start a project" link on the site behind the curtain also opens the modal
const siteContactLink = document.querySelector('[data-testid="site-open-contact"]')
if (siteContactLink) {
  siteContactLink.addEventListener('click', (e) => {
    e.preventDefault()
    openContact()
  })
}
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    if (contactModal.classList.contains('is-active')) closeContact()
  }
})
