...
- **Center back arrow → smooth reverse-fade to cube**. New `is-fading-back` CSS class drives a 1.1 s opacity tween. `backToCube()` is now async — adds `is-exiting` (triggers per-element transitions for elements not driven by letter FX) + `is-fading-back` (overall opacity), awaits `playWelcomeExit()` (letter swirl out on all 5 elements in parallel), waits 250 ms for the opacity tween to complete, then clears all state classes. Visual result: the welcome dissolves elegantly back to reveal the cube — the inverse of the entrance ripple-then-letter-swirl.
- **CSS guard**: when `welcome-overlay.has-letterfx`, the simple opacity-translate transitions on greet + actions are suppressed (override to opacity 1 + transform none). They'd otherwise fight with the letter-FX-driven span animations. Pure-CSS path remains for users who haven't enabled letter FX (still a smooth fade).
- **Verified**: `greetSpans=10`, `headingSpans=11`, `ctaPrimarySpans=15`, `ctaSecondarySpans=13`, `ctaPrimaryHasLetterFx=True` after entrance. Mid-flight screenshots show all 5 elements simultaneously animating at every transition (entrance, scroll-down threshold, scroll-up threshold, center-back). Lint clean.


User feedback: "add a tiny FastAPI route that injects published settings as a `<script>window.__INITIAL_SETTINGS__ = {...}</script>` before sending index.html. P2 — Replace auto-generated og-image.png with a designer-rendered version."

- **Server-side settings injection** — replaced the static `serve` host with a tiny custom Express server (`/app/frontend/serve.js`, ~95 LOC). On every `GET /` (and the SPA fallback for unknown paths), the server reads `dist/index.html`, fetches the latest `/api/settings` from the local backend (1.5 s AbortController hard cap so a slow backend never blocks the page), and injects `<script>window.__INITIAL_SETTINGS__={...}</script>` immediately before `</head>`. The inline hydration script in `<head>` was upgraded to prefer `window.__INITIAL_SETTINGS__` (server-rendered) over `localStorage` (cached) so EVERY visitor — including first-timers on a new device — sees published values from the very first paint, no flash. Cache-Control `no-store, must-revalidate` on the HTML so admin Publish is reflected on next reload. Static assets (JS bundles, images) still served by `express.static` with the usual cache headers. Backend was NOT modified — k8s ingress routes `/` to port 3000, so the injection had to live on the frontend side. (Implementation chosen over a true FastAPI rewrite because k8s ingress would need to be reconfigured to route `/` to backend, which we can't do in-cluster.)
- **Designer-rendered OG/Twitter card** — replaced the auto-generated `og-image.png` with a Pillow-rendered editorial card at `/app/scripts/build_og_image.py`. 1200 × 630 PNG, ~124 KB. Layout: black background with a subtle hairline grid (editorial print feel), a soft radial accent glow in the top-right corner (matches the cube's signature), top-left "● JAX STUDIO" brand chip, massive stacked **JAX** (white) / **STUDIO** (orange-outlined) display title, thin orange rule + small-caps tagline "GRAPHIC DESIGN · BRAND IDENTITY · 2026", and a "● COMING SOON" bottom-right caption. Hairline corner brackets frame the card. AI image-analysis confirmed: "designer-rendered, modern, slightly futuristic, professional, with strong visual hierarchy and intentional composition". Re-buildable any time via `python3 /app/scripts/build_og_image.py`.
- **Test coverage**: 41/41 backend pytest pass (no backend changes, all previous tests still green). Frontend verified by `testing_agent_v3_fork` — SSR injection (curl + Playwright `window.__INITIAL_SETTINGS__` inspection), Cache-Control no-store, custom-published values rendered from first paint with zero flash, OG bytes + dimensions + meta tag, static assets served correctly, SPA fallback returns HTML with injection.
